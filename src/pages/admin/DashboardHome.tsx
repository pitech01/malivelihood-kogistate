    ...
    return (
        <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
                <StatCard title="Total Applicants" value="1,245" change="+12%" color="#3b82f6" icon="👥" />
                <StatCard title="Pending Review" value="452" change="+5%" color="#f59e0b" icon="⏳" />
                <StatCard title="Approved" value="320" change="+8%" color="#10b981" icon="✅" />
                <StatCard title="Rejected" value="45" change="-2%" color="#ef4444" icon="❌" />
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '2rem' }}>
                ...
                <div style={{ background: '#111', borderRadius: '8px', border: '1px solid #222', padding: '1.5rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                        ...
                        <button style={{ background: 'none', border: 'none', color: '#3b82f6', fontSize: '0.9rem', cursor: 'pointer' }}>View All</button>
                    </div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <thead>
                            <tr style={{ borderBottom: '1px solid #222', textAlign: 'left' }}>
                                ...
                                <th style={{ padding: '0.75rem 0', color: '#666', fontSize: '0.85rem', fontWeight: 500 }}>Date</th>
                                <th style={{ padding: '0.75rem 0', color: '#666', fontSize: '0.85rem', fontWeight: 500 }}>Status</th>
                            </tr>
                        </thead>
                        <tbody>
                            {[
                                { name: 'Ibrahim Musa', lga: 'Lokoja', date: 'Oct 24', status: 'Pending' },
                                { name: 'Sarah Adebayo', lga: 'Okene', date: 'Oct 24', status: 'Approved' },
                                { name: 'Emmanuel Ojo', lga: 'Kabba', date: 'Oct 23', status: 'Rejected' },
                                { name: 'Fatima Yusuf', lga: 'Dekina', date: 'Oct 23', status: 'Pending' },
                                { name: 'John Doe', lga: 'Idah', date: 'Oct 22', status: 'Approved' },
                            ].map((row, idx) => (
                                <tr key={idx} style={{ borderBottom: idx < 4 ? '1px solid #222' : 'none' }}>
                                    <td style={{ padding: '1rem 0', fontSize: '0.9rem', color: 'white', fontWeight: 500 }}>{row.name}</td>
                                    <td style={{ padding: '1rem 0', fontSize: '0.9rem', color: '#888' }}>{row.lga}</td>
                                    <td style={{ padding: '1rem 0', fontSize: '0.9rem', color: '#888' }}>{row.date}</td>
